<template>
  <AppLayout>
    <div class="calls-view">
      <!-- Header -->
      <div class="calls-header">
        <div class="header-content">
          <h1 class="page-title">
            <v-icon class="me-2">mdi-phone</v-icon>
            {{ $t('calls.title') }}
          </h1>
          <p class="page-description">
            {{ $t('calls.description') }}
          </p>
        </div>
        <div class="header-actions">
          <v-btn
            color="primary"
            variant="elevated"
            prepend-icon="mdi-plus"
            @click="showCreateDialog = true"
            class="create-call-btn"
          >
            <span class="btn-text-desktop">{{ $t('calls.newCall') }}</span>
            <span class="btn-text-mobile">{{ $t('common.create') }}</span>
          </v-btn>
        </div>
      </div>

      <!-- Call Statistics -->
      <CallStats :stats="callsStore.callStats" />

      <!-- Filters -->
      <CallFilters
        :filters="callsStore.filters"
        @update:filters="callsStore.updateFilters"
      />

      <!-- View Toggle and Sort -->
      <div class="view-controls">
        <div class="view-info">
          <v-chip variant="outlined" size="small">
            {{ callsStore.filteredCalls.length }} {{ $t('calls.unit') }}
          </v-chip>
        </div>

        <div class="sort-controls">
          <v-select
            v-model="sortBy"
            :items="sortOptions"
            item-title="label"
            item-value="value"
            :label="$t('common.sortBy')"
            @update:modelValue="applySorting"
            class="sort-dropdown"
            density="comfortable"
            variant="outlined"
          />
        </div>
      </div>

      <!-- Loading State -->
      <div v-if="callsStore.loading && callsStore.filteredCalls.length === 0" class="loading-container">
        <ListSkeleton
          :count="5"
          avatar
          actions
          type="list-item-three-line"
        />
      </div>

      <!-- Error State -->
      <v-alert
        v-else-if="callsStore.error"
        type="error"
        class="error-message"
        closable
        variant="tonal"
      >
        {{ callsStore.error }}
        <template #append>
          <v-btn
            icon="mdi-refresh"
            size="small"
            variant="text"
            @click="loadCalls"
          />
        </template>
      </v-alert>

      <!-- Empty State -->
      <div v-else-if="callsStore.filteredCalls.length === 0" class="empty-state">
        <div class="empty-content">
          <v-icon class="empty-icon">mdi-phone</v-icon>
          <h3>{{ $t('calls.empty.title') }}</h3>
          <p v-if="hasActiveFilters">
            {{ $t('calls.empty.noFilter') }}
          </p>
          <p v-else>
            {{ $t('calls.empty.noData') }}
          </p>
          <div class="empty-actions">
            <v-btn
              v-if="hasActiveFilters"
              color="secondary"
              variant="outlined"
              prepend-icon="mdi-filter-off"
              @click="callsStore.clearFilters"
            >
              {{ $t('common.clearFilters') }}
            </v-btn>
            <v-btn
              color="primary"
              variant="elevated"
              prepend-icon="mdi-plus"
              @click="showCreateDialog = true"
              class="create-call-btn"
            >
              <span class="btn-text-desktop">{{ $t('calls.createCall') }}</span>
              <span class="btn-text-mobile">{{ $t('common.create') }}</span>
            </v-btn>
          </div>
        </div>
      </div>

      <!-- Calls Content -->
      <div v-else class="calls-content">
        <div class="calls-grid">
          <CallCard
            v-for="call in sortedCalls"
            :key="call.id"
            :call="call"
            @edit="editCall"
            @delete="confirmDeleteCall"
          />
        </div>
      </div>

      <!-- Call Form Dialog -->
      <CallForm
        v-model="showCreateDialog"
        :loading="formLoading"
        @submit="createCall"
        @cancel="showCreateDialog = false"
      />

      <CallForm
        v-model="showEditDialog"
        :call="selectedCall"
        :loading="formLoading"
        @submit="updateCall"
        @cancel="showEditDialog = false"
      />

      <!-- Delete Confirmation Dialog -->
      <v-dialog
        v-model="showConfirmDialog"
        max-width="400px"
        persistent
      >
        <v-card>
          <v-card-title class="text-h6">
            {{ $t('common.confirmDelete') }}
          </v-card-title>

          <v-card-text>
            {{ $t('calls.deleteConfirm') }}
            {{ $t('common.irreversible') }}
          </v-card-text>

          <v-card-actions>
            <v-spacer />
            <v-btn
              color="secondary"
              variant="outlined"
              @click="cancelDelete"
            >
              {{ $t('common.cancel') }}
            </v-btn>
            <v-btn
              color="error"
              variant="elevated"
              @click="confirmDelete"
            >
              {{ $t('common.delete') }}
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </div>
  </AppLayout>
</template>

<script setup lang="ts">
import AppLayout from "@/components/layout/AppLayout.vue"
import CallCard from "@/components/calls/CallCard.vue"
import CallFilters from "@/components/calls/CallFilters.vue"
import CallForm from "@/components/calls/CallForm.vue"
import CallStats from "@/components/calls/CallStats.vue"
import { ListSkeleton } from "@/components/skeletons"
import { useSnackbar } from "@/composables/useSnackbar"
import { useCallsStore } from "@/stores/calls"
import { useI18n } from "vue-i18n"
import type {
  Call,
  CallCreateRequest,
  CallUpdateRequest,
} from "@medical-crm/shared"
import { computed, onMounted, ref } from "vue"

const callsStore = useCallsStore()
const { showSnackbar } = useSnackbar()
const { t } = useI18n()

// Component state
const sortBy = ref("createdAt")
const showCreateDialog = ref(false)
const showEditDialog = ref(false)
const selectedCall = ref<Call | null>(null)
const formLoading = ref(false)

// Confirmation dialog state
const showConfirmDialog = ref(false)
const callToDelete = ref<Call | null>(null)

const sortOptions = computed(() => [
  { label: t('calls.sort.byDate'), value: "createdAt" },
  { label: t('calls.sort.byType'), value: "callType" },
  { label: t('calls.sort.byPhone'), value: "phoneNumber" },
  { label: t('calls.sort.byDuration'), value: "duration" },
])

const hasActiveFilters = computed(() => {
  return Object.keys(callsStore.filters).some(
    (key) =>
      callsStore.filters[key as keyof typeof callsStore.filters] !== undefined &&
      callsStore.filters[key as keyof typeof callsStore.filters] !== null &&
      callsStore.filters[key as keyof typeof callsStore.filters] !== ""
  )
})

const sortedCalls = computed(() => {
  const calls = [...callsStore.filteredCalls]

  return calls.sort((a, b) => {
    switch (sortBy.value) {
      case "createdAt":
        return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()

      case "callType":
        const typeOrder = { missed: 0, incoming: 1, outgoing: 2 }
        return typeOrder[a.callType] - typeOrder[b.callType]

      case "phoneNumber":
        return a.phoneNumber.localeCompare(b.phoneNumber)

      case "duration":
        const aDuration = a.duration || 0
        const bDuration = b.duration || 0
        return bDuration - aDuration

      default:
        return 0
    }
  })
})

const loadCalls = async () => {
  try {
    await callsStore.fetchCalls()
  } catch (error) {
    showSnackbar(t('calls.errors.load'), "error")
  }
}

const createCall = async (callData: CallCreateRequest) => {
  try {
    formLoading.value = true
    await callsStore.createCall(callData)
    showCreateDialog.value = false
    showSnackbar(t('calls.success.created'), "success")
  } catch (error) {
    showSnackbar(t('calls.errors.create'), "error")
  } finally {
    formLoading.value = false
  }
}

const editCall = (call: Call) => {
  selectedCall.value = call
  showEditDialog.value = true
}

const updateCall = async (updates: CallUpdateRequest) => {
  if (!selectedCall.value) return

  try {
    formLoading.value = true
    await callsStore.updateCall(selectedCall.value.id, updates)
    showEditDialog.value = false
    selectedCall.value = null
    showSnackbar(t('calls.success.updated'), "success")
  } catch (error) {
    showSnackbar(t('calls.errors.update'), "error")
  } finally {
    formLoading.value = false
  }
}

const confirmDeleteCall = (call: Call) => {
  callToDelete.value = call
  showConfirmDialog.value = true
}

const deleteCall = async (call: Call) => {
  try {
    await callsStore.deleteCall(call.id)
    showSnackbar(t('calls.success.deleted'), "success")
  } catch (error) {
    showSnackbar(t('calls.errors.delete'), "error")
  }
}

const confirmDelete = () => {
  if (callToDelete.value) {
    deleteCall(callToDelete.value)
  }
  showConfirmDialog.value = false
  callToDelete.value = null
}

const cancelDelete = () => {
  showConfirmDialog.value = false
  callToDelete.value = null
}

const applySorting = () => {
  // Sorting is handled by the computed property
}

onMounted(async () => {
  await loadCalls()
})
</script>

<style scoped>
.calls-view {
  padding: 2rem;
  background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);
  min-height: 100vh;
}

/* Header */
.calls-header {
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  margin-bottom: 2rem;
  background: white;
  padding: 2rem;
  border-radius: 16px;
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);
  border: 1px solid rgba(0, 0, 0, 0.04);
  position: relative;
  overflow: hidden;
}

.calls-header::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  height: 4px;
  background: linear-gradient(90deg, #10b981, #3b82f6, #ef4444);
}

.header-content {
  flex: 1;
}

.page-title {
  margin: 0 0 0.5rem 0;
  font-size: 1.75rem;
  font-weight: 600;
  color: #1f2937;
  display: flex;
  align-items: center;
  gap: 0.5rem;
}

.page-description {
  margin: 0;
  color: #6b7280;
  font-size: 1rem;
}

.header-actions {
  display: flex;
  gap: 0.75rem;
}

.create-call-btn {
  white-space: nowrap;
}

/* Button text responsive */
.btn-text-mobile {
  display: none;
}

.btn-text-desktop {
  display: inline;
}

/* View Controls */
.view-controls {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 2rem;
  background: white;
  padding: 1.5rem;
  border-radius: 12px;
  box-shadow: 0 2px 12px rgba(0, 0, 0, 0.06);
  border: 1px solid rgba(0, 0, 0, 0.04);
}

.view-info {
  display: flex;
  align-items: center;
  gap: 0.5rem;
}

.sort-controls {
  display: flex;
  align-items: center;
  gap: 0.75rem;
}

.sort-dropdown {
  min-width: 200px;
}

/* Loading and Error States */
.loading-container {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  padding: 4rem 2rem;
  background: white;
  border-radius: 16px;
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);
  border: 1px solid rgba(0, 0, 0, 0.04);
  text-align: center;
}

.loading-container p {
  margin-top: 1rem;
  color: #6b7280;
  font-size: 1.1rem;
}

.error-message {
  margin-bottom: 2rem;
  border-radius: 12px;
}

/* Empty State */
.empty-state {
  background: white;
  border-radius: 16px;
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);
  border: 1px solid rgba(0, 0, 0, 0.04);
  padding: 4rem 2rem;
  text-align: center;
}

.empty-content {
  text-align: center;
  max-width: 400px;
  margin: 0 auto;
}

.empty-icon {
  font-size: 3rem;
  color: #d1d5db;
  margin-bottom: 1rem;
}

.empty-content h3 {
  margin: 0 0 0.75rem 0;
  font-size: 1.25rem;
  font-weight: 600;
  color: #374151;
}

.empty-content p {
  margin: 0 0 1.5rem 0;
  color: #6b7280;
  line-height: 1.5;
}

.empty-actions {
  display: flex;
  justify-content: center;
  gap: 0.75rem;
  flex-wrap: wrap;
}

/* Calls Content */
.calls-content {
  background: white;
  border-radius: 16px;
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);
  border: 1px solid rgba(0, 0, 0, 0.04);
  overflow: hidden;
  padding: 2rem;
}

.calls-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(380px, 1fr));
  gap: 1.5rem;
}

/* Responsive Design */
@media (max-width: 1200px) {
  .calls-grid {
    grid-template-columns: repeat(auto-fill, minmax(340px, 1fr));
  }
}

@media (max-width: 1024px) {
  .calls-view {
    padding: 1.5rem;
  }

  .calls-grid {
    grid-template-columns: repeat(auto-fill, minmax(320px, 1fr));
  }
}

@media (max-width: 768px) {
  .calls-view {
    padding: 0.75rem;
  }

  .calls-header {
    padding: 1.25rem;
    margin-bottom: 1.25rem;
    flex-direction: column;
    gap: 1rem;
    text-align: center;
  }

  .calls-header::before {
    height: 3px;
  }

  .header-actions {
    justify-content: center;
    width: 100%;
  }

  .create-call-btn {
    width: 100%;
    max-width: 250px;
  }

  .page-title {
    font-size: 1.5rem;
    justify-content: center;
  }

  .calls-content {
    padding: 1.25rem;
  }

  .calls-grid {
    grid-template-columns: 1fr;
    gap: 1rem;
  }

  .view-controls {
    padding: 1rem;
    margin-bottom: 1.25rem;
    flex-direction: column;
    gap: 1rem;
  }

  .view-info {
    justify-content: center;
  }

  .sort-controls {
    justify-content: center;
    width: 100%;
  }

  .sort-dropdown {
    width: 100%;
  }
}

@media (max-width: 640px) {
  .calls-view {
    padding: 0.5rem;
  }

  .calls-header {
    padding: 1rem;
    border-radius: 12px;
    flex-direction: column;
    gap: 1rem;
    text-align: center;
  }

  .header-content {
    order: 1;
  }

  .header-actions {
    order: 2;
    justify-content: center;
    width: 100%;
  }

  .create-call-btn {
    width: 100%;
    max-width: 200px;
  }

  /* Switch button text on mobile */
  .btn-text-desktop {
    display: none;
  }

  .btn-text-mobile {
    display: inline;
  }

  .calls-content {
    padding: 0.75rem;
  }

  .loading-container,
  .empty-state {
    padding: 2rem 1rem;
    border-radius: 12px;
  }

  .view-controls {
    flex-direction: column;
    gap: 1rem;
    padding: 1rem;
  }

  .sort-dropdown {
    min-width: auto;
  }
}

@media (max-width: 480px) {
  .calls-view {
    padding: 0.25rem;
  }

  .empty-actions {
    flex-direction: column;
    gap: 0.75rem;
  }

  .calls-header {
    padding: 0.75rem;
    margin-bottom: 1rem;
  }

  .page-title {
    font-size: 1.25rem;
    flex-direction: column;
    gap: 0.25rem;
  }

  .page-description {
    font-size: 0.9rem;
  }

  .loading-container,
  .empty-state {
    padding: 1.5rem 0.75rem;
  }

  .calls-content {
    border-radius: 12px;
  }

  .calls-grid {
    gap: 0.75rem;
  }

  .create-call-btn {
    font-size: 0.9rem;
    padding: 0.75rem 1rem;
  }
}
</style>
